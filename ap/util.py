...
def freeze(cont):
    ...
    Recursively replaces all mutable containers (`dict`, `list`, `set`) encountered within `cont` by an immutable equivalent:
    Lists are converted to tuples, sets to `frozenset` objects, and dictionaries to tuples of (*key*, *value*) pairs.
    """
